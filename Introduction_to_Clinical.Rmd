...
```{r setup}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("/home1/TO/analysis_data")) 
```
...
```{r}
write.csv(clinic, "/home1/bojialiu/490_Cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
# 1. How many rows are there? --How many columns? there are 1097 rows and 112 columns
# 2. What is the data type of $gender? --it is a string 
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)?--around 18371/365=50.332  years
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer this --she is (20211 - 3767)/365 =45.0521 years old
...
plot(clinic$age_at_diagnosis, clinic$lymph_nodes_examined_count,
     main = "Age at Diagnosis vs Lymph Nodes Examined",
...
```{r Exercise 4.2}
boxplot(age_at_diagnosis ~ vital_status,
        data = clinic,
...
```{r}
under30_over60_mask <- clinic$birth_days_to < 30*365 | clinic$birth_days_to > 60*365
```
...
cleaned_clinic$age_category <- ifelse(cleaned_clinic$age_at_diagnosis <= 35, "Young",
                                      ifelse(clinic$age_at_diagnosis >=50, "Old", "Middle"))
```
...
15*365=5,475, at this many days the survival rate for "Old" patients is around 30%
...
p = 0.2, it means that age group does not significantly effect survival. 
